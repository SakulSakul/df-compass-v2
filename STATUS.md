...
**현재 Phase: 2 — 컷오버 발효 보류 (2026-08-06 사용자 차단 의결)** ·
UI 패리티 정비 진행 → 감리·승인 후 발효 재상정 (기충족 조건·롤백 계획 유효,
v1 = backup/v1-3.6-final 보존·운영 계속)
...
## 작업 로그 (최신이 위)
- 2026-08-17(2) **지시서 010-r1 완료 — P2′ 라우터 조건부 리랭크·발효 보류 해제**: 감리자 의결(자격 창 폐기·예약석 확장 기각) 이행 — retrieve() 무지목 시 리랭크 스킵→RRF 순서 top_k(지목 시 현행 유지, router=None 무영향, trace `rerank_skipped` 계장). 게이트 3조건 전부 통과: (a) 단위 4건 신설(스킵/수행/미구성/하류 불변) **119 passed** (b) 골든 52 재실측(52/52·에러 0, rank_trace_20260817T130946) — 무관 무지목 6건 승격 문서 진입 **0(g47 오염 소멸)**·rescue 38문항/71청크(기준선 36/66 ±10% 내)·g14/g12/g09/g13 유지·관련 무지목 0건(판정 원시 p2prime_gateb_verdict) (c) negative 마커 **6/6**(기준선 5/6, 전건 스킵, neg_reject_20260817T131312). 부기: g50 지목→무지목 변동(무해), g09 5위 CREDO↔징계기준 교대는 스킵 무관 리랭커 동점 변동. **009 G3′ 발효 보류 해제.** 보고 docs/reports/010-보고.md.
- 2026-08-17 **지시서 010 P1 완료·회부 정지**: retrieve 내부 3층 계장(관측 전용·RPC 인덱스 캡처) + 골든 52 전수 운영 계기 실측(52/52·에러 0·115 passed). **판정 재료가 창 전제를 반증** — 관련 질문 45 중 36문항(80%)에서 RRF>10 rescue 실재(청크 66개·RRF 11~25), 승격 문서 rescue 14건(g14 12→1 등). 무관 그룹은 g47 1건만 진입(운영 계기 RRF 15→2). 차단 상한(N≤14)과 편익 하한(N≥25)이 교차 불가 — 순위 창 단독으론 오염·편익 분리 불가능 구조 실측. 사전 고정 규칙대로 N 인상 없이 정지·회부(예약석 확장 재상정 = 감리자 안건). P2 미착수.
- 2026-08-16(6) **지시서 002-A 마감 + 009 완료(③b 계열 종결)**: 002-A — 정본 CSV 1,082행 무편집 등재·게이트 활성 113 passed skip 0·처분 집계 일치(커밋 ba3af68). 009 — CREDO 승격: A0(원문 부재→유일 저장본 22청크)·무결성(조문 4종 원장 전수 ok·장 1~5·중복/빈 0)·PROXY_DOCS 공집합(전 기제 휴면 유지, 로직 제거 금지 — 감리자 즉답)·테스트 갱신(합성 명단 monkeypatch 3건+휴면 실증 신설 1건) **114 passed**. D: g13 HIGH — CREDO 조번호 인용이 원장 대조 통과(승격 효과 완전 실증), live_card 는 CREDO 검색 미진입(승격 실패 아님 구분 명기). **G3′ 오염 관측 미통과** — g47(점심 메뉴)에서 징계기준 RRF 14위→리랭크 1위→최종 컨텍스트 진입 1건. 사전 고정 규칙대로 **랭킹 보정 별도 안건 상정·발효 보류 사유** 보고. 명문 개정 1줄 기재. 나머지 6문항·CREDO 전건 미진입.
- 2026-08-16(5) **지시서 008 완료 + 007 완료(P0 통과) + 002-A 블로커 지속**: 008 — A0 실측(원문 컬럼 부재·storage 0·DOCX 부재 → 유일 저장본=청크 13) → 해석 규칙 발동, 청킹 무결성 검사(수위 8종 전부·항목 1~40 결번 0·중복/빈 0) → 승격 커밋 b314620(PROXY_DOCS 1건 제거·CREDO 잔존, 형제 grep 완료·프록시 테스트는 CREDO 기준이라 갱신 불요). D 검증: g14 등급 불변(C-class 앵커 0 — 정상)·live1 거래처 선물 **HIGH**(MEDIUM 상한 해제 실증)·3문항 전부 징계기준 진입/인용/⚖️. 007 — P0 프로브: Q1~Q3 **3/3 리랭크 풀 진입**(Q2·Q3 rank 1) → 사전 고정 규칙 통과, Q4 양성 rank 1·Q5 음성 21위 원시 기록. 008 선완료 명기. 002-A — 첨부 CSV 이 세션 미도착(전 경로 탐색 0건) — 재전송 대기. **111 passed**.
- 2026-08-16(4) **지시서 006 완료** — 등급 라벨 상단 복원(전 등급 nx-grade 캡슐 + 하단 메타 행 이중 표시, v1 정합): 002-C "상단 중복 0" 지시서 결함 판정(규약 2조 첫 발동 — 구현자 무책)의 정정 이행. UNVERIFIED 중립 분기·LOW 강등 칩·block 무접촉. **111 passed**(3등급×상하단 어서션). 엔진 0줄. 디자인 축 마감 — 감리 검산·사용자 육안 대기.
- 2026-08-16(3) **지시서 005 완료** — 참고 사규 카드 v1 정합(render.py:299-328 구조+styles nx-doc-* 원문 이식): 카드+배지(doc_kind 미탑재 → 문서명 결정론 매핑: 징계기준/사규/DOC)+조번호 우측(article_no 파서 검증값만 — 환각 0)+스니펫 480자 전부 escape, 구 형식 폴백. **111 passed**·라이브 DOM 실측(카드 5·배지·cite 0 정합)+메타 행 🤖 gemini-3.7-flash 확인. 매트릭스 시각 구조 사각 보완 기록. 엔진 0줄.
- 2026-08-16(2) **지시서 003 완료** — 3.7 Flash A/B 실측 러닝 4건(base 1차·재실행·A·B) 등재: 규칙 1 교집합 재현율 base 51/51·A 51/51·B 50/51 / 규칙 4 절단률 합산 base 7/104 vs A+B 0/104 / 위조 비강등 전런 0 / A 전체 재현율 53/53·p50 10.9s(base 17.7s)·thoughts 토큰 절반. 모델 ID gemini-3.7-flash 실확인·env 주입 기구현(엔진 0줄)·비용 개산 내 실측. docs/reports/003-보고.md — 감리 독립 재계산·판정 대기.
- 2026-08-16 **지시서 004 완료** — 합성 기본값 gemini-3.7-flash 전환(_DEFAULT_CHAT_MODEL 상수 통일, 커밋 7b04870, 롤백=revert). 운영 경로 직배선 스모크 PASS(provider 3.7·HIGH·12.1s — 3.6 동일 질문 18~19s 참고 관측)·110 passed 무회귀. 검증 방법 결함 1건 자기 신고: 하니스 스모크는 v1_sidecar 의 env setdefault(3.6) 오염 — 운영 경로와 무관, 003 러닝은 명시 주입이라 무오염 확인. 003 A/B 러닝(base 결측 5=폴백 키 부재, A·B 진행 중)은 별도 완료 보고 예정.
- 2026-08-10 **지시서 001·002 이행**(워크플로우 규약 v2 — 보고 파일 docs/reports/ 병행 개시): 001 완료 — 기구현(f1b3f7b)을 판정식 정합화(_inflight={q,ts}·[ui:abort] 태그), 수용 기준 5항 자체 검증 전항 PASS(라이브 재실측 포함). 002 부분 완료 — B(SRMS 렌더 직전 결정론 치환·이중 치환 금지·저장 원문 불변, 라이브 a[href] 5개 실측)·C(HIGH/MEDIUM 어휘 메타 행 통합 [카테고리·등급·⏱️·🤖]·상단 카테고리 뱃지 제거·UNVERIFIED/LOW 현행 — 등급 상하단 중복 제거 해석 보고서 명기)·D(히어로 placeholder v1 원문 바이트 일치)·게이트 테스트 등재(전건 처분+IMPLEMENT 잔존 0 어서션). **A 블로커: 원장 CSV 본문 미수신**(지시서에 첨부 명시됐으나 릴레이에 부재) — skip 사유 명시로 등재, 수신 시 마감 커밋 1건. **110 passed 2 skipped**. 엔진 0줄.
- 2026-08-09(3) **중단 복구 간소안 구현**(심의 의결 — 표시 계층 전용): inflight 마커를 질문 말풍선과 동일 수명 매체(session_state)에 기록(생존성 등가 — 서버 재시작이면 질문 자체가 소실돼 배너 불요) → 잔존+댕글링 user 턴 감지 시 "직전 답변 생성이 중단되었습니다" 배너+[이 질문 다시 실행](pending_q 단일 경로·1회 한정 멱등·rerun_done 마커 소거 선행) / 정상 완료·의도된 중지(⏹)는 마커 소거(허위 배너·중복 배너 방지) / 중단 카운터 stderr 로그([ui:interrupt] — 30일 관측 데이터, ADR-8 준수). **107 passed**(배너·멱등·허위 배너 부재·중지 억제 4어서션)·라이브 실증(처리 중 selectbox 값 변경으로 run 킬 → 배너 → 재실행 완주 → 멱등 True). 엔진 0줄. **파리티 원장 IMPLEMENT 3건은 항목 명세 미릴레이 — 수신 대기.**
- 2026-08-09(2) **DOCK 인사 폴백 제거**(트랙C nomatch 오안내 판정 — 오안내<무안내 복원): _CAT_DOCK 에서 "인사" 삭제 — hr dock 은 v1 정합대로 답변 내 인사 안내 문구(_A_HR_KW) 신호로만 발동, 컨텍스트 결정론은 공정거래·재무·CSR 유지. 어서션 개정(인사 카테고리 단독→hidden / 인사 문구 답변→hr) **104 passed**. 하니스 개선: answer 스냅샷을 피드백 위젯 안착 대기 후로 교정 + 부분 재실행 인자. **3문항 재러닝(6/6 완주)**: fair_trade·nomatch 전항 PASS(아티팩트 3건 소거+오안내 수정 실증), hr 만 DIFF 3 잔존(v1=답변 문구 신호로 hr dock 발동, v2=해당 문구 부재로 미발동 — 인사 폴백 제거의 직접 결과, 문면 의존. ghost ±1 은 검출기 한계 기판정) — 판정은 감리자. 엔진 0줄.
- 2026-08-09 **트랙C E2E 비교 하니스 구축·러닝 완료**(tools/e2e_compare/run_compare.py): v1·v2 로컬 동시 기동(NEXUS_ENV=prod-e2e — 양쪽 동의 게이트 비활성 동일 조건)·6문항(공정거래/재무/인사/안전/CSR/무매칭)×상태별(홈/처리중 2s 샘플×3/답변) DOM 체크리스트 8항 동일 기준 판정. 1차 러닝 전멸 원인 규명(Streamlit DOM 에 stMain testid 부재 → stMainBlockContainer 로 교정) 후 2차 **12/12 완주·에러 0**. 원시 결과: 셀 PASS 108/DIFF 12(전부 answer 상태 — dock_labels 5·dock_present 3·ghost_text 3·feedback 1). 판정 없이 무편집 전달 — 감리자 독립 재계산 대기. JSON 은 repo 등재, 스크린샷 62장 tar 별도 전달.
- 2026-08-08(3) **트랙A 실결함 4건 수정**: ①푸터(v1 :1885-1893 원문, 빈 홈·대화 공통) ②히스토리 렌더 윈도우 30+생략 캡션(v1 :1698-1707 — 성능 가드 겸, idx 절대값 유지) ③무매칭 방어(검색 0건 → 합성 미호출·v1 문안 기반 안내 영속 엔트리 — v2 도 RPC 0건 구조적 가능, 근거 없는 생성 방지) ④일시 장애 특화 안내(503/429/RESOURCE_EXHAUSTED/high demand 신호 → v1 :1112-1121 문안, 그 외 범용 유지). **104 passed**(어서션 4건). 엔진 0줄. 트랙C 하니스 착수.
...
- 2026-08-07(4) **동시 제출 가드 완결**(감리 3.5/4 → ④ 잔여 지적): 하단 chat_input 인라인 처리 제거 — 제출 run 은 pending_q 적재+rerun(v1 :1924-1929), 처리 run 은 항상 _q_processing=True 잠금 프레임에서 합성. 이식 중 v1 대조로 결손 1건 추가 발견·동반 이식: 처리 완료 후 **마무리 st.rerun()**(v1 :1913) — 없으면 잠금 프레임이 잔존해 입력 교착(라이브에서 실측된 결함). 오류 표면은 rerun 생존을 위해 영속 notice_error 엔트리로 전환(st.error 렌더 유지). **98 passed**·라이브 실측: 처리 중 disabled 5/5 True → 완료 후 False·재질문 완주. 엔진 0줄. 정지 → 배포 반영 → URL 포함 실물 스크린샷 육안 대기.
- 2026-08-07(3) **빈 홈 입력창 이중화 해소**(사용자 감리 판정 — v1 은 병존이 아님): v1 상태 기계 그대로 이식 — ①chat_input 항상 호출 유지 + 빈 홈 렌더 블록 내부에서만 stChatInput display:none 주입(미렌더 시 자동 원복, 조건부 분기 없음) ②빈 홈 = st.empty() placeholder + 게이트에 question(하단 q_input+pending_q 합류) 포함 ③히어로 제출은 pending_q 단일 경로 유지 ④`_q_processing` 가드 이식(pending_q 처리 run 은 chat_input disabled, 히어로는 게이트로 미렌더 — 동시 제출의 진행 run 폐기 원천 차단). **97 passed**(상태 기계·잠금 어서션 3건 추가)·라이브 실측: 빈 홈 chat_input visible False → 히어로 Enter 제출 → 1문답 후 visible True·히어로 부재. 검증 스크립트 오탐 1건 규명(첫 input 타겟이 사이드바 selectbox — 앱 결함 아님). 엔진 0줄. 정지 → 감리 커밋 실물 대조 대기.
- 2026-08-07(2) **F 계급(시간축·모션) 전수 이식**(매트릭스 사각 인정 — 정지 화면 기반 대조가 모션 계급째 누락): F-1 ①실시간 경과 타이머(v1 iframe JS 이식, 실측 1→8초 증가+완료 시 정리) ②단계 전환 스피너(분석→탐색→작성[근거 사규 중간 노출 실측 @12s]→검증 — status_cb 는 app 계층 파이프라인 호출 사이에만, 엔진 0줄. v1 실물의 progress bar 는 CSS 숨김이라 스피너+단계 텍스트가 정본) ③⏹ 생성 중지(플래그→rerun 중단→결과 폐기→안내+toast, 라이브 실측 True) ④예상 소요 고지(정적 20~60초 — avg_latency 는 query_logs 부재로 대체)+keyframes 3종·THINKING 블록 이식. F-2 전수 추출·판정: 동의어 st.info 제외(엔진 신호 부재)/피드백 실패 toast 해당 없음/모드 toast 백로그/verified_ask 배지 제외(실험 분기). **동반 이식(발견 갭 회신)**: 히어로 Enter 제출(on_change)·피드백 (B) 사유 pills+자유 의견+제출/건너뛰기(라벨 v1 원문 정정). **95 passed**·연속 캡처 6장+단계 전환 4장+중지 1장(모두 실측 로그 동반). 정지 → 감리 검산 대기.
- 2026-08-07 **UI 결함 4건 일괄 수정**(사용자 감리 적발 — 표시 계층 한정, compass_engine 0줄): ①UNVERIFIED 라벨 분기 — 인용 표기 존재(cite_n 또는 [참조:]·「」·계약형 결정론 탐지; check_document_names 는 통과 항목 미기록이라 cite_n 단독 불가)·비강등 → 중립 "사규 문서 대조 확인", 인용 부재·강등만 강한 경고 유지. 문서수준 MEDIUM 신설 필요성 실증 사례로 백로그 심의 자료 기록 ②③빈 불릿·빈 번호 — 근본원인: **.streamlit/config.toml 누락**(v1 은 base=light 고정, v2 이식 누락)으로 다크 모드 브라우저에서 emotion 텍스트 색 rgb(250,250,250)이 주입 CSS 라이트 배경 위에 얹혀 백색-on-백색 소실(다크 재현 + computed color 실측 → 테마 고정 후 rgb(61,60,56) 실증. 조립·데이터 무결은 격리 앱 재현으로 배제) — 형제 전부(피드백 문구·클로징·유도 캡션 동일 계열) 일괄 해소 ④DOCK 오분기 — 근본원인: v1 confidence="low" fallback 을 grade UNVERIFIED 로 대응시켜 C-class(76/101) 답변 전반이 hr 오발동 → 답변 명시 신고 신호+카테고리 결정론 매핑(재무→경리팀·경영관리팀 신설/인사→인사교육팀/CSR→클린신고)으로 재설계, 질문 휴리스틱·low fallback 제거, 무매핑=주 액션 생략. **91 passed**·법인카드 전체 스크롤 4장+다크 모드 실증 캡처·정지 → 감리자 검산 대기.
- 2026-08-06 UI 패리티 **최종 마감 라운드**(통합 지시서): ① 검산 지적 2건 이식 — 피드백 캡션 "베타 단계입니다…"(feedback.py:314) + PROACTIVE DOCK "다음 단계 예측"(feedback.py:112-140 — grep 0건 보고는 검색 범위 한정 오류로 정정 기록, classify_button 다중 신호 축약+패널 3종+🔄 다시 답변) ② 사용자 판정 2건 이식 — 베타 동의서 게이트(_consent_gate, v1 문안·검증 regex·prod 비활성. 쿠키/DB 기록은 세션+stderr 대체 — ADR-8) + 비용 가드(_check_rate_limit, KST 리셋·한도 100·notice 영속) ③ 신규 "이해 확인+구체화 유도" 표면(사용자 설계) — _is_fragment_followup 파편·지시어 감지 → 검색 미실행+재진술+단일 턴 안내+직전 답변 followups 자동 제출 칩, no-op 폴백 AppTest 고정, 일반 답변 하단 "보다 구체적인 답변을 위해…" 유도 캡션 ④ 관련 질문 모드(엔진형) 백로그 확정. 매트릭스 갱신(누락 0·승인 대기 0)·**88 passed**·스크린샷 6종(consent/home/normal/coach/critical/OOS — critical·OOS 는 DOCK 반영 재캡처, 라이브 검수: 클린신고/신고 분기 정확). 잔여 = 사용자 최종 육안 1회 → 발효 재상정.
- 2026-08-06 UI 패리티 **전수 매트릭스 방식 전환**(반응형 중단·감리자 인벤토리): 피드백 실종 규명(현 코드 존재·무조건 렌더 — 감리 화면이 885d3a0 이전 빌드인 시차, 회귀 아님) → 누락분 일괄 구현(피드백 v1 플로우 문구·카테고리 뱃지·참고 사규·검토 캡션·⏱🤖·💡·클로징/시간대 인사·에러 표면) → docs/ui-parity-matrix.md 작성: **누락 0**, 의도적 제외 3(동의서 게이트·비용 가드·관련 질문 모드 — 사용자 승인 대기) + 다음 단계 예측 근거 미발견(감리자 회신 요청) + 피드백 영속화 ADR-8 기이관. 84 passed·상태별 스크린샷 4종(홈/일반/critical/OOS)·critical 라이브 계약형 인용 실측.
- 2026-08-06 UI 패리티 잔여 보정 완료(사용자 감리 지적 3건): ① 타이핑 재생 — 검증 후 재생(st.write_stream, 실 토큰 스트리밍 금지 명기·안전 표면 즉시 표시·히스토리는 정적) ② 랜딩 v1 시각 정합 — 54px 히어로+레드 물음표·로고 라인+베타 칩·전폭 방패 배너·중앙 pill 입력+원형 레드 전송·pill 예시 칩 (computed style 기계 확인: 54px·rgb(200,16,46)·50%·999px) ③ ::after 마침표 오타 근본 수정+회귀 테스트. 84 passed·critical 스모크 공존·엔진 diff 0. 배포 push 완료 — 사용자 육안 재확인 대기(랜딩 첫인상·답변 타이핑).
- 2026-08-06 **UI 패리티 정비 완료·감리 대기**: app.py 표시 계층 확장 — 판정 카드(v1 색조·세리프 인용·출처)·conf 4등급 라벨 UI(심의 3항 기이행)·'참고 인용' 구획(표시만)·출처 문서 칩·🧠 검토 과정·베타 배너·v1 히어로 문안·예시 6·질의 범위 필터·응답 시간·피드백(세션+stderr, 영속화는 ADR-8 심의 후)·후속 제안(grounded SELECT). 엔진 무접촉 실증(diff=app.py·tests만)·안전 표면 3불변 AppTest 고정·84 passed·라이브 스모크 3(critical 배너 공존)·골든 스팟 5/5·스크린샷 3장 제출(마커 결함 1건 발견·수정 재캡처). v1 스크린샷은 프록시 차단 — 감리자 확보 요청.
- 2026-08-06 **컷오버 발효 보류**(사용자 차단 의결 — 공지 전 무비용 정지) + 감리자 정정(07-30 스크린샷 앱 귀속 오독: 히어로 화면=v1). 안건 = UI 패리티 정비(판정 카드·conf 4등급 라벨 UI[심의 3항 기이행 의무]·검토 과정·히어로/예시 칩·베타 배너·질의 범위·응답 시간·피드백·후속 제안). 범위 = 표시 계층 한정·엔진 무접촉·안전 표면 불변·판정식/채점기/DB쓰기/v1 동결. 착수.
- 2026-08-06 **컷오버 발효**(감리자 선언·정본 docs/cutover-effective-20260806.md): CG-2·3·4 전건 충족. 라이브 — Claude 폴백 체인 최초 실증(배포 키 확인으로 R3 한계 해소)·절단 재발 1(백로그)·경미 오타 1. v1 3.6 보존 표식 = backup/v1-3.6-final(c326f40, 태그 프록시 차단 → 런북 폴백). OBSERVATION 30일 규약 전환. 잔여 = 사내 공지 발송[사용자]. 최종 성적: 재현율 58.8→96.2%·위조 비강등 0·홀드아웃 3/3·19.7s.
