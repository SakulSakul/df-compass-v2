...
- [ ] G2 성격전환 — 파생 원장 커버리지 인간 표본 감사 [사용자 액션]
      ...
      **필수 편입: "(인사) 직장 내 괴롭힘 예방·대응지침"** (2026-07-27 후속
      조사 — 원장 [부칙] 1건뿐, 제N조 표기 자체가 없는 번호 항목 문서)
      **방식 변경 (2026-07-28 사용자 승인)** — 기계 전수 대조(Cowork 감리자,
      파서·원장 무관 독립 채널) + 인간 표본 재확인(무작위 5건 + 괴롭힘 지침)
      으로 진행. 원 설계(인간 전수 표본)보다 커버리지는 전수로 강화, 인간
      검증은 표본 수준이라는 성격 변경 명시. 착수는 후순위 유지.
      지원 킷: tools/g2_ledger_dump.py → eval/results/g2_ledger_dump_*.md
      (문서별 등재 조항 전수 + 시드 고정 표본 추첨)
