...
- [ ] **컷오버 전제 — 사용자용 UI 정비** (2026-07-28 사용자 지시 등재).
      현 v2 UI 는 Phase 1 ⑨ 의도대로 빌더 전용 프리뷰라 일반 공개 부적합.
      컷오버 풀 심의(~8/10) 전에 v1 동등 이상으로 정비: Warm Editorial 완전
      이식 · 빌더 배지/개발용 표시 제거 · 일반 사용자 화면 구성.
      **불변 조건 (위반 시 H 정지)**: verify block 미표출 · degrade 칩 ·
      critical 배너+4단+핫라인 박스의 표출 로직·가시성 유지.
      범위: 엔진(compass_engine) 무변경 — app.py 표현 계층만.
      완료 검증 = deploy-notes 3층 체크리스트 재실행 + 안전 표면 육안 확인.
      phase-2.md 컷오버 조건에 "사용자용 UI 준비" 추가 여부는 심의에서 결정
      — 현 시점은 백로그 등재만 (착수 지시 대기).
