...
_A_HR_KW = ("인사 규정·복리후생 등 인사 행정 사항은 인사교육팀에 문의",
            ...
            "인사팀에 문의해 주시기 바랍니다")
# 카테고리 결정론 매핑 — "인사" 폴백 없음 (트랙C nomatch 오안내 판정):
# v1 hr dock 은 답변 내 인사 안내 문구(_A_HR_KW) 신호로만 발동하고, 컨텍스트
# 결정론은 공정거래 1개로 의도적으로 좁혀져 있다(over-trigger 방지).
# 무관련 질문도 하이브리드 검색이 최근접 청크를 반환하므로 카테고리 폴백
# 확장은 오안내를 만든다 — 오안내<무안내.
_CAT_DOCK = {"재무": "finance", "CSR": "clean", "공정거래": "report"}
