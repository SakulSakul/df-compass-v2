...
| 추천 칩 (pill) | home.py:9-13·16-32·109-126 + core/faq_cache.py:264-288 | 이식됨 (08-07 v1 정합 재이식) | ~~6개 대형 카드~~ → **정확히 3개 알약형**(36px·999px·13px — computed 기계 확인): 짧은 라벨 ↔ 전체 질의 매핑, faq_cache(show_on_home) SELECT(ttl=300)+폴백 상수 3쌍 v1 원문, 클릭 = pending_q 단일 경로. 기존 "존재만 대조, 개수·형태·데이터 소스 미대조" 감리 누락 정정 |
